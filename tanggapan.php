...
try {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
        ...
        $stmt = $conn->prepare("UPDATE data_beasiswa SET nama = :nama, email = :email, nomor = :nomor, semester = :semester, nilai = :nilai, pilih_beasiswa = :pilih_beasiswa, status = :status WHERE user_id = :user_id");
...
<body>
    <div class="container mt-5">
        <h2 class="text-center">Proses Data Beasiswa</h2>
        <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
            <input type="hidden" name="user_id" value="<?php echo htmlspecialchars($result['user_id']); ?>">
            <div class="mb-3">
                <label for="nama" class="form-label">Nama</label>
                <input type="text" class="form-control" id="nama" name="nama" value="<?php echo htmlspecialchars($result['nama']); ?>" readonly required>
            </div>
            <div class="mb-3">
                <label for="email" class="form-label">Email</label>
                <input type="email" class="form-control" id="email" name="email" value="<?php echo htmlspecialchars($result['email']); ?>" readonly required>
            </div>
            <div class="mb-3">
                <label for="nomor" class="form-label">Nomor</label>
                <input type="text" class="form-control" id="nomor" name="nomor" value="<?php echo htmlspecialchars($result['nomor']); ?>" readonly required>
            </div>
            <div class="mb-3">
                <label for="semester" class="form-label">Semester</label>
                <input type="text" class="form-control" id="semester" name="semester" value="<?php echo htmlspecialchars($result['semester']); ?>" readonly required>
            </div>
            <div class="mb-3">
                <label for="nilai" class="form-label">IPK</label>
                <input type="number" class="form-control" id="nilai" name="nilai" value="<?php echo htmlspecialchars($result['nilai']); ?>" step="0.01" min="0" max="4.0" readonly required>
            </div>
            <div class="mb-3">
                <label for="pilih_beasiswa" class="form-label">Pilihan Beasiswa</label>
                <input type="text" class="form-control" id="pilih_beasiswa" name="pilih_beasiswa" value="<?php echo htmlspecialchars($result['pilih_beasiswa']); ?>" readonly required>
            </div>
            <div class="mb-3">
